            ...
            <div class="flex flex-col gap-3">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <input required value="{{ $event->responsable }}" type="text" name="responsable"
                            id="responsable"
                            class="w-full px-4 py-2 transition bg-white border rounded-lg shadow-sm border-primary-300"
                            @unlessrole('admin') disabled @endunlessrole>
                    </div>
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <select required name="location" id="location"
                            ...
                            <option value="nacional" @if ($event->location == 'nacional') selected @endif>Nacional
                            </option>
                            <option value="internacional" @if ($event->location == 'internacional') selected @endif>
                                Internacional
                            </option>
                        ...
                        <select required id="university" class="w-full" name="universities[]" multiple
                            ...
                            @foreach ($universities as $university)
                                <option value="{{ $university->id }}"
                                    @if (isset($event->universities) && in_array($university->id, $event->universities->pluck('id')->toArray())) selected @endif>
                                    {{ $university->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="flex flex-col gap-3">
                        ...
                        <select name="activity_id" id="activity_id"
                            ...
                            @foreach ($activities as $activity)
                                <option value="{{ $activity->id }}"
                                    @if ($event->activity_id == $activity->id) selected @endif>
                                    {{ $activity->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1">
                    <div class="flex flex-row items-center justify-end gap-2">
                        ...
                        @endhasrole
                        @hasrole('regen')
                            <form method="POST" action="{{ route('events.approveCertificate', $event->id) }}">
                                @method('put')
                                @csrf
                                <button type="submit"
                                    class="inline-block px-4 py-2 font-semibold text-white transition rounded-lg shadow-md bg-gradient-to-bl to-primary-700 from-primary-500 hover:scale-95">
                                    Aprobar Evento para Certificación
                                </button>
                            </form>
                        @endhasrole
