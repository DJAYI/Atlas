<x-layouts.dashboard-layout>
    ...
    <form action="{{ route('universities.update', $university->id) }}" class="flex flex-col gap-6" method="POST">
        ...
        @php
            $readonly = !auth()->user()->can('edit universities') ? 'readonly' : '';
            $disabled = !auth()->user()->can('edit universities') ? 'disabled' : '';
        @endphp
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full px-4 py-2 transition bg-white border border-primary-300 rounded-lg shadow-sm resize-none"
                        {{ $readonly }}>{{ $university->description }}</textarea>
                </div>
